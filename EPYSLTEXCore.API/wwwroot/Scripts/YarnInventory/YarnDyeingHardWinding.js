(function () {
    ...
    function initMasterTable() {
        ...
        if (status == statusConstants.PENDING) { // Data get to the booking table
            columns = [
                ...
                {
                    field: 'YDBatchNo', headerText: 'YD Batch No', width: 100, textAlign: 'left'
                },
                {
                    field: 'YDBookingNo', headerText: 'YD Booking No', width: 110, textAlign: 'left'
                },
                {
                    field: 'YDBookingDate', headerText: 'YD Booking Date', type: 'date', format: _ch_date_format_1, width: 100, textAlign: 'center'
                },
                {
                    field: 'YDBNo', headerText: 'YDB No', width: 100, textAlign: 'left'
                },
                {
                    field: 'ConceptNo', headerText: 'Concept No', width: 120, textAlign: 'left'
                },
                {
                    field: 'TotalBookingQty', headerText: 'Total Booking Qty', width: 100, textAlign: 'right'
                },
                {
                    field: 'ProducedQty', headerText: 'Produced Qty', width: 150, textAlign: 'right'
                },
                {
                    field: 'Qty', headerText: 'Hard Winding Qty', width: 150, textAlign: 'right'
                },
                //{
            ...
            var additionalColumns = [
                {
                    field: 'HardWindingNo', headerText: 'Hard Winding No', width: 150
                },
                {
                    field: 'HardWindingDate', headerText: 'Hard Winding Date', type: 'date', format: _ch_date_format_1, width: 150
                },
                {
                    field: 'Buyer', headerText: 'Buyer', width: 150
                },
                //{
                //    field: 'ReceiveQty', headerText: 'Receive Qty', width: 150
                //},
                {
                    field: 'Qty', headerText: 'Hard Winding Qty', width: 150
                },
                //{
        ...
        $tblMasterEl = initEJ2Grid({
            tableId: tblMasterId,
            apiEndPoint: `/api/yarn-dyeing-hard-winding/list/${status}/${pageName}`,
            columns: columns,
    ...
    function handleCommands(args) {
        ...
        if (status === statusConstants.PENDING) {
            getNew(args.rowData.YDBookingMasterID, args.rowData.YDBatchID);
        }
        else {
            getDetails(args.rowData.HardWindingMasterID);
        }
    }
    ...
    async function initChildTable(data) {
        ...
        if (isRequisitionPage) {
            ...
            var additionalColumns = [
                ...
                { field: 'YDDryerFinishingChildID', headerText: 'YDDryerFinishingChildID', allowEditing: false, visible: false },
                ...
                { field: 'BacthQty', headerText: 'Batch Qty', allowEditing: false },
                { field: 'BatchCone', headerText: 'Batch Cone', allowEditing: false },
                { field: 'ReceiveCarton', headerText: 'Batch Carton', allowEditing: false, visible: false },
                { field: 'DryerFinishQty', headerText: 'Dryer Finish Qty', allowEditing: false },
                { field: 'DryerFinishCone', headerText: 'Dryer Finish Cone', allowEditing: false },
                { field: 'Qty', headerText: 'Hard Winding Qty', editType: "numericedit", edit: { params: { showSpinButton: false, decimals: 2, min: 1 } } },
            ...
            var additionalColumns = [
                ...
                { field: 'DryerFinishQty', headerText: 'Dryer Finish Qty', allowEditing: false },
                { field: 'DryerFinishCone', headerText: 'Dryer Finish Cone', allowEditing: false },
                { field: 'Qty', headerText: 'Hard Winding Qty', allowEditing: true, editType: "numericedit", edit: { params: { showSpinButton: false, decimals: 2, min: 1 } } },
    ...
    function getNew(id, ydBatchID) {
        axios.get(`/api/yarn-dyeing-hard-winding/new/${id}/${ydBatchID}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                masterData = response.data;
                masterData.HardWindingNo = masterData.HardWindingNo;
                masterData.HardWindingDate = formatDateToDefault(masterData.HardWindingDate);
    ...
    function getDetails(id) {
        axios.get(`/api/yarn-dyeing-hard-winding/${id}`)
            .then(function (response) {
    ...
    function save(IsSendForApprove) {
        ...
        axios.post("/api/yarn-dyeing-hard-winding/save", data)
            .then(function () {
    ...
    function isValidChildForm(data) {
        ...
        $.each(data["Childs"], function (i, el) {
            if (el.Qty == "" || el.Qty == null || el.Qty <= 0) {
                toastr.error("Hard Winding Qty is required.");
                isValidItemInfo = true;
            }
            else if (el.Qty > el.ReceivedQty) {
                toastr.error("Hard Winding Qty must be equal or less than Receive Qty");
                isValidItemInfo = true;
            }
        });
