(function () {
    ...
    $(function () {
        ...
        $tblYarnEl = $("#tblKnittingPlanBDSYarn" + pageId);
    ...
    function initMasterTable() {
        ...
        $tblMasterEl = new initEJ2Grid({
            tableId: tblMasterId,
            apiEndPoint: `/api/knitting-program/list?type=${knittingProgramType.BDS}&status=${status}`,
            columns: columns,
            hasChildGrid: true,
            autofitColumns: status !== statusConstants.PENDING,
            childOptions: {
                queryString: "BookingID",
                apiEndPoint: `/api/knitting-program/booking-child-list/${knittingProgramType.BDS}`,
                allowPaging: false,
                ...
                childOptions: {
                    queryString: "SubGroupID",
                    additionalQueryParams: "BookingID",
                    apiEndPoint: `/api/knitting-program/booking-child-details-list/${status}/${knittingProgramType.BDS}`,
                    allowPaging: false,
                    allowFiltering: false,
    ...
    function handleCommands(args) {
        if (status === statusConstants.PENDING && !args.rowData.KnittingPlanned) {
            getNew(args.rowData.BookingID,args.rowData.ItemMasterID);
        }
        else {
    ...
    function backToList() {
        ...
        $("#divKnittingPlanBDSYarn").fadeOut();
        $tblMasterEl.refresh();
    }
    ...
    function getNew(bookingId, itemMasterId) {
        ...
        axios.get(`/api/knitting-program/new/${knittingProgramType.BDS}/${bookingId}/${itemMasterId}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
    ...
    function getDetails(id) {
        ...
        axios.get(`/api/knitting-program/${knittingProgramType.BDS}/${id}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                masterData = response.data;
    ...
    function save(isApprove = false) { 
        ...
        for (var i = 0; i < masterData.Childs.length; i++) {
            ...
            if (masterData.Childs[i].KJobCardMasters.length == 0) {
                kJobCardMaster = {
                    ...
                    SubGroupID: masterData.Childs[i].SubGroupID,
                ...
                for (var j = 0; j < masterData.Childs[i].KJobCardMasters.length; j++) {
                    ...
                    masterData.Childs[i].KJobCardMasters[j].SubGroupID = masterData.Childs[i].SubGroupID;
        ...
        data.KnittingProgramType = knittingProgramType.BDS;
