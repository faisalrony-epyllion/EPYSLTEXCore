    ...
    internal class SendToYDStoreService : ISendToYDStoreService
    {
        ...
        public async Task<List<SendToYDStoreMaster>> GetPagedAsync(Status status, PaginationInfo paginationInfo)
        {
            ...
            if (status == Status.Pending)
            {
			   ...
			   FROM {TableNames.YD_BATCH_ITEM_REQUIREMENT} IR
			   ...
			   --INNER JOIN {TableNames.YD_DRYER_FINISHING_CHILD} DFC ON DFC.YDBItemReqID = IR.YDBItemReqID
			   --INNER JOIN {TableNames.HardWindingChild} HWC ON HWC.YDBItemReqID = IR.YDBItemReqID
			   INNER JOIN {TableNames.YD_QC_CHILD} QCC ON QCC.YDBItemReqID = IR.YDBItemReqID
			   ...
			   N AS (
					...
					LEFT JOIN {TableNames.SendToYDStoreChild} YDSC ON YDSC.YDBItemReqID = IR.YDBItemReqID
            ...
            else if (status == Status.Draft)
            {
                sql += $@";WITH M AS (
                    ...
                    FROM {TableNames.SendToYDStoreMaster} QM
        ...
        public async Task<SendToYDStoreMaster> GetNewAsync(int newId)
        {
            var sql = $@" ;WITH M AS (
					...
					From X
					INNER JOIN {TableNames.YD_QC_CHILD} QCC ON QCC.YDBItemReqID = X.YDBItemReqID
					...
					From X
					INNER JOIN {TableNames.SendToYDStoreChild} YDSC ON YDSC.YDBItemReqID = X.YDBItemReqID
                    ...
                    FROM X
					INNER JOIN {TableNames.YD_QC_CHILD} YDQC ON YDQC.YDBItemReqID = X.YDBItemReqID
        ...
        public async Task<SendToYDStoreMaster> GetAsync(int id)
        {
            var sql = $@";WITH M AS (
                        ...
                        FROM {TableNames.SendToYDStoreMaster} QM WHERE QM.SendToYDStoreMasterID = {id}
                    )
                    ...
                    ;WITH X AS (
                        ...
                        FROM {TableNames.SendToYDStoreChild} YDSC WHERE YDSC.SendToYDStoreMasterID = {id}
                    )
        ...
        {
            var sql = $@"
            ;Select * FROM {TableNames.SendToYDStoreMaster} Where SendToYDStoreMasterID = {id}

            ;Select * FROM {TableNames.SendToYDStoreChild} Where SendToYDStoreMasterID = {id}";
