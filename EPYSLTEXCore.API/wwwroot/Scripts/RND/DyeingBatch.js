(function () {
        ...
        axios.get(`/api/dyeing-batch/get-batch-details/${batchIds}`)
            .then(function (response) {
                for (var i = 0; i < response.data.length; i++) {
                    ...
                    for (var j = 0; j < response.data[i].DyeingBatchRecipes.length; j++) {
                        $tblRecipeChildE1.getCurrentViewRecords().push(response.data[i].DyeingBatchRecipes[j]);
                    }
                }
    ...
    function initMasterTable() {
        ...
        var columns = [
            ...
            { field: 'DBatchNo', headerText: 'D. Batch No', width: 20, visible: status == statusConstants.COMPLETED },
            { field: 'DBatchDate', headerText: 'D. Batch Date', textAlign: 'Center', width: 20, type: 'date', format: _ch_date_format_1, visible: status == statusConstants.COMPLETED },
            ...
            { field: 'RecipeDate', headerText: 'Recipe Date', visible: false, textAlign: 'Center', width: 20, type: 'date', format: _ch_date_format_1 },
        ...
        $tblMasterEl = new initEJ2Grid({
            ...
            apiEndPoint: `/api/dyeing-batch/list?status=${status}`,
            columns: columns,
    ...
    function initChildTable(data) {
        ...
        $tblChildEl = new ej.grids.Grid({
            ...
            columns: [
                ...
                */
                { field: 'BatchID', isPrimaryKey: true, visible: false },
                { field: 'BatchNo', headerText: 'Batch No', width: 40, allowEditing: false },
            ...
            actionBegin: function (args) {
                if (args.requestType === "save") {
                    ...
                    var totalBatchWeightKG = 0,
                        totalBatchQtyPcs = 0;
                    list.map(x => {
                        if (x.BatchID == args.rowData.BatchID) {
    ...
    function initItemChildTable(data) {
        ...
        $tblItemChildE1 = new ej.grids.Grid({
            ...
            columns: [
                 ...
                 */
                { field: 'BItemReqID', isPrimaryKey: true, visible: false },
                ...
                { field: 'Qty', headerText: 'Qty (Kg)', width: 15, allowEditing: false },
                { field: 'QtyPcs', headerText: 'Qty (Pcs)', width: 15, allowEditing: false }
            ],
            childGrid: {
                ...
                columns: [
                    ...
                    { field: 'DBIRollID', headerText: 'DBIRollID', textAlign: 'Center', width: 20, allowEditing: false, visible: false },
                ...
                toolbarClick: function (args) {
                    ...
                    if (args.item.id === "addItem") {
                        ...
                        setSubGroupValue(masterData.DyeingBatchItemRolls);
                        var ChildItemsFabric = masterData.DyeingBatchItemRolls.filter(function (el) {
                        ...
                        var ChildsFabric = masterData.DyeingBatchItems;
                        ...
                        if (ChildItemsFabric.length > 0) {
                            ...
                            if (typeof childObj.DyeingBatchItemRolls !== "undefined" && childObj.DyeingBatchItemRolls != null) {
                                SFDChildRollIDs = childObj.DyeingBatchItemRolls.map(x => x.SFDChildRollID).join(",");
                            }
                            ...
                            var finder = new commonFinder({
                                ...
                                onMultiselect: function (selectedRecords) {
                                    ...
                                    ChildsFabric[indexFind].DyeingBatchItemRolls = FabricItemList;
                                    //ChildsFabric[indexFind].ChildItems = FabricItemList;
                                    ChildsFabric[indexFind].Qty = iDCQty;
                                    ////var indexFindMD = masterData.DyeingBatchItems.findIndex(x => x.BItemReqID == data.BItemReqID);
                                    ////masterData.DyeingBatchItems[indexFindMD].DyeingBatchItemRolls = FabricItemList;
                                    ////masterData.DyeingBatchItems[indexFindMD].Qty = iDCQty;
    ...
    function showRoll(rollId) {
        ...
        if (rollId > 0) {
            var ChildItemsFabric = masterData.DyeingBatchItemRolls.filter(x => x.GRollID == rollId);
            if (ChildItemsFabric.length > 0) {
        ...
        axios.post("/api/batch/save-kProd", rows)
            .then(function (response) {
                ...
                response.data.newSplitedList.map(x => {
                    ...
                    var index = masterData.DyeingBatchItemRolls.findIndex(y => y.GRollID == x.GRollID);
                    if (index < 0) {
                        index = masterData.DyeingBatchItemRolls.findIndex(y => y.GRollID == x.ParentGRollID);
                        var hasList = masterData.DyeingBatchItemRolls != null && masterData.DyeingBatchItemRolls.length > 0 ? true : false;
                        x.SFDChildRollID = x.GRollID;
                        x.SFDChildID = hasList ? masterData.DyeingBatchItemRolls[index].BItemReqID : 0;
                        x.RollQty = x.RollQty;
                        x.GRollID = x.GRollID;
                        x.BChildID = hasList ? masterData.DyeingBatchItemRolls[index].BChildID : 0;
                        x.BItemReqID = hasList ? masterData.DyeingBatchItemRolls[index].BItemReqID : 0;
                        x.BatchID = hasList ? masterData.DyeingBatchItemRolls[index].BatchID : 0;
                        x.BatchNo = hasList ? masterData.DyeingBatchItemRolls[index].BatchNo : 0;
                        x.RollNo = hasList ? masterData.DyeingBatchItemRolls[index].RollNo : "";
                        x.SubGroupID = hasList ? masterData.DyeingBatchItemRolls[index].SubGroupID : _subGroup;
                        x.SubGroupName = hasList ? masterData.DyeingBatchItemRolls[index].SubGroupName : getSubGroupName(x.SubGroupID);
                        x.GroupConceptNo = hasList ? masterData.DyeingBatchItemRolls[index].GroupConceptNo : "";
                        x.ConceptID = hasList ? masterData.DyeingBatchItemRolls[index].ConceptID : 0;
                        x.CCColorID = hasList ? masterData.DyeingBatchItemRolls[index].CCColorID : 0;
                        x.ColorID = hasList ? masterData.DyeingBatchItemRolls[index].ColorID : 0;
                        masterData.DyeingBatchItemRolls.push(x);
                        //masterData.DyeingBatchItems.find(y => y.BItemReqID == x.BItemReqID).DyeingBatchItemRolls.push(x);
                        if (index > -1) {
                            if (indexList.includes(index)===false) {
                                indexList.push(index);
                            }
                            //masterData.DyeingBatchItemRolls.splice(index, 1);
                        }
                    }
                });
                indexList.map(index => {
    ...
    function populateChildex(childId, $detail) {
        ...
        var childIndex = getIndexFromArray(masterData.DyeingBatchRecipes, "RecipeDInfoID", childId)
        initDefChildTable(childId, $detail);
        var cnt = (masterData.DyeingBatchRecipes[childIndex].DefChilds == null) ? 0 : masterData.DyeingBatchRecipes[childIndex].DefChilds.length;
        if (cnt > 0)
            $tblDefChildEl.bootstrapTable('load', masterData.DyeingBatchRecipes[childIndex].DefChilds.filter(function (item) {
    ...
    function getNew(batchId) {
        axios.get(`/api/dyeing-batch/new/${batchId}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                ...
                masterData.DyeingBatchItems = setQtyRollWise(masterData.DyeingBatchItems);
                childs = setQtyBatchWise(childs, masterData.DyeingBatchItems);

                initChildTable(childs);
                initItemChildTable(masterData.DyeingBatchItems);
                initRecipeChildTable(masterData.DyeingBatchRecipes);
            })
            .catch(function (err) {
                toastr.error(err.response.data.Message);
            });
    ...
    function getDetails(id) {
        axios.get(`/api/dyeing-batch/${id}`)
            .then(function (response) {
                ...
                initChildTable(masterData.DyeingBatchWithBatchMasters);
                initItemChildTable(masterData.DyeingBatchItems);
                initRecipeChildTable(masterData.DyeingBatchRecipes);
            })
            .catch(function (err) {
    ...
    function save() {
        ...
        data.DyeingBatchWithBatchMasters = $tblChildEl.getCurrentViewRecords();//masterData.FinishingProcessChilds;
        data.DyeingBatchItems = $tblItemChildE1.getCurrentViewRecords();
        data.DyeingBatches = selectedMasterItems;
        if (data.DyeingBatchItems.length == 0) {
            toastr.error("No Item Found.");
            return;
        }
        var childs = [];
        masterData.DyeingBatchRecipes.forEach(function (child) {
        ...
        data.DyeingBatchRecipes = childs;
        data.SLNo = masterData.SLNo;
        data.IsRework = isRework;

        axios.post("/api/dyeing-batch/save", data)
            .then(function () {
                toastr.success("Saved successfully!");
                backToList();
            })
    ...
    function createNew() {
        ...
        var batchIds = selectedRecords.map(x => x.BatchID).join(",");
        ...
        if (isNew && !isMultiSelect) {
            ...
            axios.get(`/api/dyeing-batch/new/multiSelect/${batchIds}`)
                .then(function (response) {
                    $divDetailsEl.fadeIn();
                    $divTblEl.fadeOut();
                    ...
                    result.map(x => {
                        ...
                        dyeingBatchItems.push(...x.DyeingBatchItems);
                        dyeingBatchRecipes.push(...x.DyeingBatchRecipes);
                    });

                    dyeingBatchItems = setQtyRollWise(dyeingBatchItems);
                    childs = setQtyBatchWise(childs, dyeingBatchItems);

                    initChildTable(childs);
                    initItemChildTable(dyeingBatchItems);
                    initRecipeChildTable(dyeingBatchRecipes);
    ...
    function setQtyBatchWise(listItems, dyeingBatchListItems) {
        var batchWeightKG = 0,
            batchQtyPcs = 0;
        ...
        listItems.map(x => {
            ...
            dyeingBatchListItems.filter(y => y.BatchID == x.BatchID).map(y => {
