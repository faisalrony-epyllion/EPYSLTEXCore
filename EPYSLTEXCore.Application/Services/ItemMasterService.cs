...
namespace EPYSLTEXCore.Application.Services
{
    public class ItemMasterService<T> : IItemMasterService<T> where T : BaseItemMaster
    {
        ...
        private int AddItemInTempItemMaster(ref List<ItemMasterBomTemp> newItemMasters, List<ItemMasterBomTemp> itemMasters, ItemSubGroupDTO subGroup, BaseItemMaster baseItem)
        {
            try
            {
                int maxId = 0;
        ...
        private bool RegisterItemMaster(ref List<ItemMasterBomTemp> newItemMasters, ItemSubGroupDTO objItemSubGroup)
        {
            try
            {
        ...
        private ItemSubGroupDTO GetItemSubGroup(int subGroupId)
        {
            ...
            {
                 _connection.Open();
                return  _connection.QueryFirstOrDefault<ItemSubGroupDTO>(query);
        ...
        private List<ItemMasterBomTemp> GetItemMasters(int subGroupId)
        {
            ...
            {
                _connection.Open();
                var records=  _connection.Query<ItemMasterBomTemp>(sql);
        ...
        private void SetSegmentValueDesc(int subGroupId, ref List<T> list)
        {
            ...
            {
                 _connection.Open();
                var records =  _connection.Query<Select2OptionModel>(CommonQueries.GetItemSegmentValuesBySubGroupId(subGroupId));
                itemSegmentValueList= records.ToList();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public Task<ItemMaster> AddAsync(ItemMaster item)
        {
        ...
        public Task<ItemMaster> GetByIdAsync(string Id)
        {
            throw new NotImplementedException();
        }

        public Task<IEnumerable<ItemMaster>> GetAllAsync()
        {
            throw new NotImplementedException();
        }

        public Task<bool> UpdateAsync(long Id, ItemMaster item)
        {
            throw new NotImplementedException();
        }

        public Task<IEnumerable<ItemSegmentValue>> AddSegmentsAsync(IEnumerable<ItemSegmentValue> entities, string tableName)
        {
            return _itemSegmentService.AddManyAsync(entities, tableName);
        }

        public IEnumerable<T> GetDataAsync<T>(string query,int db_type)
        {
            switch (db_type) {
                case 1:
                    try
