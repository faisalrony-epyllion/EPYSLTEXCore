    ...
    public class YDRecipeRequestService : IYDRecipeRequestService
    {
        ...
        public async Task<List<YDRecipeRequestMaster>> GetPagedAsync(Status status, PaginationInfo paginationInfo)
        {
            ...
            if (status == Status.Pending)
            {
               ...
               ;WITH A AS(
	                ...
	                FROM YDBookingMaster YDM
	                INNER JOIN YDBookingChild YDC ON YDC.YDBookingMasterID = YDM.YDBookingMasterID
	                INNER JOIN FreeConceptMRChild MRC ON MRC.FCMRChildID = YDC.FCMRChildID
	                INNER JOIN FreeConceptMRMaster MRM ON MRM.FCMRMasterID = MRC.FCMRMasterID
	                INNER JOIN FreeConceptMaster FCM ON FCM.ConceptID = MRM.ConceptID
            ...
            else if (status == Status.Completed)
            {
               ...
               ;WITH A AS(
                ...
                G AS (
	                ...
	                FROM YDRecipeRequestMaster RR
					LEFT JOIN YDRecipeDefinitionMaster RDM ON RDM.YDRecipeReqMasterID = RR.YDRecipeReqMasterID
            ...
            if (YDDBatchID == 0)
            {
                query =
                   $@"
                        ...
                        LEFT JOIN FreeConceptMRChild FCMRC1 ON FCMRC1.YBChildItemID = YDBC.YBChildItemID AND YDBC.YBChildItemID <> 0
						LEFT JOIN FreeConceptMRChild FCMRC2 ON FCMRC2.FCMRChildID = YDBC.FCMRChildID
						INNER JOIN FreeConceptMRMaster FCMR ON FCMR.FCMRMasterID = Case When ISNULL(FCMRC1.FCMRMasterID,0) = 0 Then FCMRC2.FCMRMasterID Else FCMRC1.FCMRMasterID END
                        INNER JOIN FreeConceptMaster FCM ON FCM.ConceptID = FCMR.ConceptID
                        LEFT JOIN FreeConceptChildColor C ON C.ConceptID = FCM.ConceptID
                        LEFT JOIN FreeConceptMaster M ON M.ConceptID = C.ConceptID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Color ON Color.SegmentValueID = C.ColorID
                        LEFT JOIN DyeingProcessPart_HK DP ON DP.DPID=C.DPID
                        ...
                        LEFT JOIN FBookingAcknowledgeChild FAC On FAC.BookingChildID = M.BookingChildID AND FAC.ConsumptionID = M.ConsumptionID AND FAC.BookingID = M.BookingID
                        ...
                        FROM YDRecipeRequestChild RC
                        LEFT JOIN FreeConceptMaster M ON M.ConceptID = RC.ConceptID
                        LEFT JOIN KnittingMachineType KnittingType ON KnittingType.TypeID=M.KnittingTypeID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Composition ON Composition.SegmentValueID=M.CompositionID
                        ...
                        ;With GC AS(
                            ...
                            From GC
                            Inner Join FreeConceptMRMaster MR On MR.ConceptID = GC.ConceptID
                            Inner Join FreeConceptMRChild MRC On MRC.FCMRMasterID = MR.FCMRMasterID
        ...
        public async Task<YDRecipeRequestMaster> GetAsync(int id, string groupConceptNo)
        {
            var query =
                ...
                FROM YDRecipeRequestMaster RM
				INNER JOIN FreeConceptChildColor C ON C.CCColorID = RM.CCColorID
				LEFT JOIN FreeConceptMaster M ON M.ConceptID = C.ConceptID
				LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Color ON Color.SegmentValueID = C.ColorID
				LEFT JOIN DyeingProcessPart_HK DP ON DP.DPID=RM.DPID
				LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Composition ON Composition.SegmentValueID=M.CompositionID
	            LEFT JOIN {DbNames.EPYSL}..Contacts CT ON CT.ContactID = M.BuyerID
	            LEFT JOIN {DbNames.EPYSL}..ContactCategoryTeam CCT ON CCT.CategoryTeamID = M.BuyerTeamID
                LEFT JOIN FBookingAcknowledgeChild FAC On FAC.BookingChildID = M.BookingChildID AND FAC.ConsumptionID = M.ConsumptionID AND FAC.BookingID = M.BookingID
				...
				FROM YDRecipeRequestChild RC
				INNER JOIN FreeConceptMaster M ON M.ConceptID = RC.ConceptID
				LEFT JOIN KnittingMachineType KnittingType ON KnittingType.TypeID=M.KnittingTypeID
                LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Composition ON Composition.SegmentValueID=M.CompositionID
                LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Construction ON Construction.SegmentValueID=M.ConstructionID
                LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Gsm ON Gsm.SegmentValueID=M.GSMID
        ...
        public async Task<List<YDRecipeRequestChild>> GetItems(string conceptNo, int colorID, int isBDS)
        {
            ...
            if (isBDS == 1)
            {
                sql = $@"With M As(
                        ...
                        TT.TechnicalName,M.Qty, M.ItemMasterId
                        From M
                        Inner Join EPYSLTEX..FreeConceptChildColor CMC ON CMC.ConceptID = M.ConceptID
                        LEFT JOIN KnittingMachineType KnittingType ON KnittingType.TypeID=M.KnittingTypeID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Composition ON Composition.SegmentValueID=M.CompositionID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Construction ON Construction.SegmentValueID=M.ConstructionID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Gsm ON Gsm.SegmentValueID=M.GSMID
            ...
            else
            {
                sql = $@"Select M.ConceptID, M.ConceptNo, M.ConceptDate, KnittingType.TypeName KnittingType, Composition.SegmentValue Composition, M.ConstructionID,
                        Construction.SegmentValue Construction, Gsm.SegmentValue GSM, M.SubGroupID SubGroupID, SG.SubGroupName SubGroup, M.TechnicalNameId,
                        TT.TechnicalName,M.Qty, M.ItemMasterId
                        From EPYSLTEX..FreeConceptMaster M
                        LEFT JOIN KnittingMachineType KnittingType ON KnittingType.TypeID=M.KnittingTypeID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Composition ON Composition.SegmentValueID=M.CompositionID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Construction ON Construction.SegmentValueID=M.ConstructionID
                        LEFT JOIN {DbNames.EPYSL}..ItemSegmentValue Gsm ON Gsm.SegmentValueID=M.GSMID
        ...
        public async Task<YDRecipeRequestMaster> GetAllByIDAsync(int ydRecipeRequestMasterID)
        {
            ...
            From YDRecipeRequestMaster A
            LEFT JOIN YDRecipeDefinitionMaster B ON B.YDRecipeReqMasterID = A.YDRecipeReqMasterID
            Where A.YDRecipeReqMasterID = {ydRecipeRequestMasterID};

            ;Select * From YDRecipeRequestChild Where YDRecipeReqMasterID = {ydRecipeRequestMasterID}

            ;Select * From YDRecipeDefinitionDyeingInfo Where YDRecipeReqMasterID = {ydRecipeRequestMasterID}";
        ...
        public async Task SaveAsync(YDRecipeRequestMaster entity)
        {
            ...
            try
            {
                ...
                {
                    string query = $"UPDATE YDBatchMaster SET YDRecipeID={entity.YDRecipeID} WHERE YDRecipeID = 0 AND GroupConceptNo = '{entity.GroupConceptNo}' AND ColorID = {entity.ColorID} AND CCColorID = {entity.CCColorID};";
                    await _batchService.ExecuteAsync(query, AppConstants.TEXTILE_CONNECTION);

                    query = $@"UPDATE A
					            SET A.RecipeID = {entity.YDRecipeID}
					            FROM YDDyeingBatchMaster A
					            LEFT JOIN YDDyeingBatchWithBatchMaster B ON B.YDDBatchID=A.YDDBatchID
                                LEFT JOIN YDBatchMaster BM ON BM.YDBatchID=B.YDBatchID AND BM.ColorID=A.ColorID
                    ...
                    query = $@"Update b Set RecipeID = c.RecipeID
                               From YDDyeingBatchMaster a
                               Inner Join YDDyeingBatchItem b On b.YDDBatchID = a.YDDBatchID
                               Inner Join YDRecipeDefinitionMaster c ON c.CCColorID = a.CCColorID and c.ColorID = a.ColorID
                               Inner Join YDRecipeRequestMaster d ON d.CCColorID = a.CCColorID and d.ColorID = a.ColorID
        ...
        {
            string sql = $@"
            ;SELECT * FROM FreeConceptChildColor WHERE CCColorID = {id}";
