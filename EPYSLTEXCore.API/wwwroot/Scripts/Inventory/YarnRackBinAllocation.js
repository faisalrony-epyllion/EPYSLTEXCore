(function () {
    ...
    function initMasterTable() {
        ...
        var columns = [
            ...
            {
                field: 'RCompanyID', headerText: 'RCompanyID', visible: false
            }
        ];
        ...
        $tblMasterEl = new initEJ2Grid({
            tableId: tblMasterId,
            autofitColumns: true,
            apiEndPoint: `/api/yarn-rack-bin-allocation/list?status=${status}&isCDAPage=${isCDAPage}`,
            columns: columns,
    ...
    function handleCommands(args) {
        if (args.commandColumn.type == 'Edit') {
            getDetails(args.rowData.ReceiveID, args.rowData.RCompanyID);
        }
    }
    ...
    function initChildTable() {
        ...
        $tblChildEl.bootstrapTable({
            ...
            onExpandRow: function (index, row, $detail) {
                populateYarnReceiveRackBin(row.ChildID, $detail);
            },
        });
    }
    ...
    function populateYarnReceiveRackBin(childId, $detail) {
        ...
        var ind = getIndexFromArray(YarnReceive.YarnReceiveChilds, "ChildID", childId)
        var cnt = YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins.length;
        if (cnt > 0)
            $el.bootstrapTable('load', YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins.filter(function (item) {
    ...
    function initYarnReceiveRackBin($el, childId) {
        ...
        $el.bootstrapTable({
            ...
            onEditableSave: function (field, row, oldValue, $el) {
                ...
                //row.EntityState = 16;
                var ind = getIndexFromArray(YarnReceive.YarnReceiveChilds, "ChildID", childId)
                var noofcart = parseFloat(YarnReceive.YarnReceiveChilds[ind].NoOfCartoon);
                var noofcon = parseFloat(YarnReceive.YarnReceiveChilds[ind].NoOfCone);
                var cnt = YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins.length;
                var noofCartAllowcated = 0
                var noofConAllowcated = 0
                for (var i = 0; i < cnt; i++) {
                    noofCartAllowcated += parseFloat(YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins[i].NoOfCartoon);
                    noofConAllowcated += parseFloat(YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins[i].NoOfCone);
                }
    ...
    window.addNewChildRow = function (childId) {
        ...
        var ind = getIndexFromArray(YarnReceive.YarnReceiveChilds, "ChildID", childId);
        var maxId = ind >= 0 ? getMaxIdForArray(YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins, "ChildRackBinID") : 1;
        var newRow = {
        ...
        YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins.push(newRow)
        $el.bootstrapTable('load', YarnReceive.YarnReceiveChilds[ind].YarnReceiveChildRackBins);
    }
    ...
    function getDetails(id, companyId) {
        axios.get(`/api/yarn-rack-bin-allocation/${id}/${companyId}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                YarnReceive = response.data;
                YarnReceive.ReceiveDate = formatDateToDefault(YarnReceive.ReceiveDate);
                YarnReceive.PODate = formatDateToDefault(YarnReceive.PODate);
                YarnReceive.PiDate = formatDateToDefault(YarnReceive.PiDate);
                YarnReceive.LCDate = formatDateToDefault(YarnReceive.LCDate);
    ...
    function save() {
        ...
        for (var i = 0; i < data.YarnReceiveChilds.length; i++) {
            ...
            if (child.YarnReceiveChildRackBins.length == 0) {
                toastr.error("Add minimum 1 Rack for each item.");
                hasError = true;
                break;
            }

            child.YarnReceiveChildRackBins = child.YarnReceiveChildRackBins.filter(x => x.EntityState != 8 && x.LocationID > 0 && x.RackID > 0 && x.EmployeeID > 0);
            if (child.YarnReceiveChildRackBins.length == 0) {
            ...
            var maxNoOfCartoon = child.NoOfCartoon;
            var maxNoOfCone = child.NoOfCone;
            ...
            for (var iRack = 0; iRack < child.YarnReceiveChildRackBins.length; iRack++) {
                var childRack = child.YarnReceiveChildRackBins[iRack];
        ...
        axios.post("/api/yarn-rack-bin-allocation/save", data)
            .then(function () {
