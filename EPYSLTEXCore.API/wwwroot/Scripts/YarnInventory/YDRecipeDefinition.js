(function () {
    ...
    $(function () {
        ...
        $formEl.find("#btnApprove").click(function (e) {
            e.preventDefault();
            var id = $formEl.find("#YDRecipeID").val();
            axios.post(`/api/yd-recipe-defination/approve/${id}`)
                .then(function () {
        ...
        $formEl.find("#btnAcknowledge,#btnUpdate").click(function (e) {
            ...
            var id = $formEl.find("#YDRecipeID").val();
            axios.post(`/api/yd-recipe-defination/acknowledge/${id}/${isArchived}`)
                .then(function () {
        ...
        $formEl.find("#btnCopyRecipe").on("click", function (e) {
            ...
            var finder = new commonFinder({
                ...
                pageId: pageId,
                apiEndPoint: `/api/yd-recipe-defination/list-by-buyer-compositon-color?dpID=${masterData.DPID}&buyer=${masterData.BuyerID}&fabricComposition=${fComp}&Color=${masterData.ColorID}`,
                isMultiselect: false,
                modalSize: "modal-md",
                top: "2px",
                primaryKeyColumn: "YDRecipeReqMasterID",
                fields: "YDRecipeNo,ColorName,ConceptNo,DPName,Buyer",
                headerTexts: "Recipe No,Color Name,Concept No,Dyeing Process,Buyer",
                widths: "50,50,30,40,50",
                onSelect: function (res) {
                    finder.hideModal();
                    axios.get(`/api/yd-recipe-defination/dyeingInfo-by-recipe/${res.rowData.YDRecipeReqMasterID}`)
                        .then(function (response) {
                            if (response.data.Childs.length > 0) {
                                ...
                                for (var i = 0; i < rows.length; i++) {
                                    ...
                                    if (rowFiber != null) {
                                        rows[i].ParentRecipeDInfoID = rows[i].YDRecipeDInfoID;
    ...
    function addNewItem() {
        var newChildItem = {
            YDRecipeChildID: getMaxIdForArray(masterData.Childs, "YDRecipeChildID"),
            EntityState: 4,
            ProcessID: 0,
            RawItemID: 0,
            ItemName: 'Empty',
    ...
    function initMasterTable() {
        var columns = [
            ...
            {
                field: 'YDRecipeNo', headerText: 'Recipe No', width: 40, visible: status !== statusConstants.PENDING
            },
            {
        ...
        $tblMasterEl = new initEJ2Grid({
            ...
            apiEndPoint: `/api/yd-recipe-defination/list?status=${status}`,
            columns: columns,
    ...
    function handleCommands(args) {
        if (status === statusConstants.PENDING) {
            if (args.commandColumn.type == 'Add') {
                ...
                getNew(args.rowData.YDRecipeReqMasterID);
            }
            ...
            if (args.commandColumn.type == 'Edit') {
                ...
                getDetails(args.rowData.YDRecipeID);
            }
    ...
    function initChildTable() {
        ...
        $tblChildEl.bootstrapTable({
            ...
            onEditableSave: function (field, row, oldValue, $el) {
                ...
                if (field === "ParticularsID") {
                    ...
                    if (selectedData) {
                        ...
                        $tblChildEl.bootstrapTable('updateByUniqueId', { id: row.YDRecipeChildID, row: row });
                    }
                }
                // Update the masterData object with the changed row data

                if (field === "Temperature") {
                    masterData.Childs.find(x => x.YDRecipeDInfoID == row.YDRecipeDInfoID).Temperature = row.Temperature;
                }
                if (field === "ProcessTime") {
                    masterData.Childs.find(x => x.YDRecipeDInfoID == row.YDRecipeDInfoID).ProcessTime = row.ProcessTime;
                }
                //var index = masterData.Childs.findIndex(function (item) { return item.YDRecipeDInfoID === row.YDRecipeDInfoID; });
            ...
            onExpandRow: function (index, row, $detail) {
                populateChild(row.YDRecipeDInfoID, $detail);
            }
        });
    }
    ...
    function populateChild(childId, $detail) {
        ...
        var childIndex = getIndexFromArray(masterData.Childs, "YDRecipeDInfoID", childId)
    ...
    function initDefChildTable(childId, $detail) {
            ...
            columnList.unshift(
                {
                    ...
                    events: {
                        'click .remove': function (e, value, rowMasterChild, indexOfChild) {
                            var indexOfMasterChild = masterData.Childs.findIndex(x => x.YDRecipeDInfoID == rowMasterChild.YDRecipeDInfoID);
                            if (indexOfMasterChild > -1 && indexOfChild > -1) {
                                masterData.Childs[indexOfMasterChild].DefChilds.splice(indexOfChild, 1);
                                $tblDefChildEl = $("#TblChildDef-" + pageId + "-" + rowMasterChild.YDRecipeDInfoID);
        ...
        $tblDefChildEl.bootstrapTable({
            ...
            onEditableSave: function (field, row, oldValue, $el) {
                ...
                ////}
                var index = masterData.Childs.find(x => x.YDRecipeDInfoID == childId).DefChilds.findIndex(function (item) { return item.YDRecipeChildID === row.YDRecipeChildID; });
                //var index = masterData.DefChildList.findIndex(function (item) { return item.id === row.id; });
                if (index !== -1) {
                    masterData.Childs.find(x => x.YDRecipeDInfoID == childId).DefChilds[index] = row;
                }
                else {
                    masterData.Childs.find(x => x.YDRecipeDInfoID == childId).DefChilds.push(row);
                }
                //masterData = JSON.parse(localStorage.getItem('masterData'));
                if (localStorage.getItem('masterData') != null) {
                    localStorage.setItem('masterData', JSON.stringify(masterData));
    ...
    window.AddDefChild = function (childId, pId, isAckPage, isAppPage) {
        ...
        //var cntChildDef = 0;
        //if (masterData != null) {
        //    cntChildDef = masterData.Childs.find(x => x.YDRecipeDInfoID == childId).DefChilds.length;
        //}
        ...
        var ind = getIndexFromArray(childs, "YDRecipeDInfoID", childId);
        if (ind > -1) {
            var newChildItem = {
                YDRecipeChildID: getMaxIdForArray(childs[ind].DefChilds, "YDRecipeChildID"),
                YDRecipeDInfoID: childs[ind].YDRecipeDInfoID,
                EntityState: 4,
                ProcessID: 0,
                RawItemID: 0,
                ItemName: 'Empty',
            ...
            newChildItem.YDRecipeChildID = newChildItem.YDRecipeChildID + 50000;
            ...
            .then(function (response) {
                showBootboxSelect2Dialog("Select Item", "RawItemID", "Select Item", response.data,
                    function (data) {
                        ...
                        $tblDefChildEl.bootstrapTable('updateByUniqueId', { id: rowData.YDRecipeChildID, row: rowData });
                    }
                    , rowData.RawItemID
                )
            })
            ...
            .then(function (response) {
                showBootboxSelect2Dialog("Select Unit", "UnitID", "Select Unit", response.data,
                    function (data) {
                        ...
                        $tblChildEl.bootstrapTable('updateByUniqueId', { id: rowData.YDRecipeChildID, row: rowData });
                    }
                    , rowData.UnitID
                )
            })
    ...
    function resetForm() {
        ...
        $formEl.find("#YDRecipeID").val(-1111);
    ...
    function getNew(id) {
        ...
        var url = "/api/yd-recipe-defination/new/" + id;
        axios.get(url)
            .then(function (response) {
    ...
    function getDetails(id) {
        ...
        axios.get(`/api/yd-recipe-defination/${id}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
    ...
    function save() {
        ...
        if (!checkValidation(data)) {
            axios.post("/api/yd-recipe-defination/save", data)
                .then(function () {
