(function () {
    ...
    async function setItems() {
        ...
        //var yarnReceiveChilds = $tblCreateItemId.getCurrentViewRecords();
    ...
    function initMasterTable() {
        ...
        else if ((isAcknowledgePage || isYDAcknowledgePage) && status != statusConstants.ACKNOWLEDGE) {
            columnsBtn = {
                headerText: 'Command', width: 100, commands: [
        ...
        var pageName = isAcknowledgePage ? pageNameConstants.MRSACK : isApprovePage ? pageNameConstants.MRSA : isYDAcknowledgePage ? pageNameConstants.YDMRSACK : "R";
        if ($tblMasterEl) $tblMasterEl.destroy();
        
        
        var url = `/api/yarn-rnd-requisition/list?status=${status}&pageName=${pageName}&isReqForYDShow=${_isReqForYDShow}`;
        $tblMasterEl = new initEJ2Grid({
            tableId: tblMasterId,
            autofitColumns: false,
            allowFiltering: true,
            apiEndPoint: `/api/yarn-rnd-requisition/list?status=${status}&pageName=${pageName}&isReqForYDShow=${_isReqForYDShow}`,
            columns: columns,
            ...
            actionBegin: function (args) {
                ...
                if (args.requestType === "save") {
                    ...
                    if (!masterData.IsReqForYD) {
                        ...
                        if (Number.isNaN(args.data.StockQty) || typeof args.data.StockQty === 'undefined') {
