(function () {
    ...
    function initMasterTable() {
        ...
        $tblMasterEl = initEJ2Grid({
            tableId: tblMasterId,
            apiEndPoint: `/api/yd-booking/list?status=${status}`,
            columns: columns,
    ...
    async function initChildTablePrv() {
        ...
        var additionalColumns = [
            ...
            { field: 'ColorBatchRef', headerText: 'Color Batch Ref' },
    ...
    async function initChildTable(data) {
        ...
        if (!isAcknowledgePage && !isApprovePage) {
            columns.push(
                {
        ...
        var additionalColumns = [
            ...
            { field: 'ColorCode', headerText: 'Color Code' },
            { field: 'ColorBatchRef', headerText: 'Color Batch Ref', allowEditing: false },
            {
                headerText: '', textAlign: 'Center', width: 40, commands: [
                    { buttonOption: { type: 'colorBatchRef', content: '', cssClass: 'btn btn-success btn-xs', iconCss: 'fa fa-search', tooltipText: "Select Batch Ref" } }
                ]
            },
            { field: 'IsTwisting', headerText: 'Twisting?', displayAsCheckBox: true, editType: "booleanedit", textAlign: 'Center' },
            { field: 'UsesIns', headerText: 'Uses In', allowEditing: false },
            ...
            {
                field: 'PerConeKG', headerText: 'KG Per Cone',
                editType: "numericedit", edit: { params: { validateDecimalOnType: true, showSpinButton: false, decimals: 2, min: 0, format: "N2" } }
            },
            { field: 'DisplayUnitDesc', headerText: 'Unit', allowEditing: false },
            { field: 'Remarks', headerText: 'Remarks' }
    ...
    async function childCommandClick(e) {
        ...
        else if (e.commandColumn.buttonOption.type == 'colorBatchRef') {
            var response = await axios.get(`/api/yd-batch/allBatchByColor/${childData.ColorID}`);
                ...
                var finder = new commonFinder({
                    title: "Select Ref Batch",
                    pageId: pageId,
                    height: 320,
                    //apiEndPoint: "/api/fabric-color-book-setups/allcolor",
                    data: response.data.Items,
                    fields: "YDBatchNo",
                    headerTexts: "YD Batch No",
                    //customFormats: "",
                    isMultiselect: false,
                    primaryKeyColumn: "YDBatchID",
                    onSelect: function (selectedRecord) {
                        finder.hideModal();
                        childData.ColorBatchRefID = selectedRecord.rowData.YDBatchID;
                        childData.ColorBatchRef = selectedRecord.rowData.YDBatchNo;
                        debugger
                        var index = $tblChildEl.getRowIndexByPrimaryKey(childData.YDBookingChildID);
                        $tblChildEl.updateRow(index, childData);
                    }
