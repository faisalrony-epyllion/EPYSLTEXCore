(function () {
            ...
            columns: [
                {
                    ...
                    formatter: function (value, row, index, field) {
                        if (status === statusConstants.PENDING) {
                            return `<a class="btn btn-xs btn-default add" href="javascript:void(0)" title="New CDA Floor Issue">
                                        <i class="fa fa-plus" aria-hidden="true"></i>
                                    </a>`;
                        }
                        else if (status === statusConstants.PARTIALLY_COMPLETED) {
                            return `<a class="btn btn-xs btn-default edit" href="javascript:void(0)" title="Edit CDA Floor Issue">
    ...
    function getMasterTableData() {
        ...
        var url = `/api/CDA-Floor-issue/list?gridType=bootstrap-table&status=${status}&${queryParams}`;
        axios.get(url)
    ...
    function getNew(FloorReqMasterId) {
        axios.get(`/api/CDA-Floor-issue/new/${FloorReqMasterId}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                CDAFloorIssue = response.data;
                CDAFloorIssue.FloorReqDate = formatDateToDefault(CDAFloorIssue.FloorReqDate);
                CDAFloorIssue.FloorIssueDate = formatDateToDefault(CDAFloorIssue.FloorIssueDate);
                setFormData($formEl, CDAFloorIssue);
                $tblChildEl.bootstrapTable("load", CDAFloorIssue.Childs);
    ...
    function getDetails(id) {
        axios.get(`/api/CDA-Floor-issue/${id}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                CDAFloorIssue = response.data;
    ...
    function save(approve = false, reject = false) {
        ...
        data["Childs"] = CDAFloorIssue.Childs;
        data.Approve = approve;
        data.Reject = reject;

        axios.post("/api/CDA-Floor-issue/save", data)
            .then(function () {
