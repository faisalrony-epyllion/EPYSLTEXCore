(function () {
    ...
    $(function () {
        ...
        $formEl.find('#DPID').on('select2:select', function (e) {
            ...
            var recipeOnList = masterData.RecipeRequestChilds.filter(x => x.RecipeOn == true);
            ...
            masterData.RecipeDefinitionDyeingInfos = [];
            if (dpName == 'Single Part' || dpName == 'Overdye') {
                ...
                var newChildItem = {
                    RecipeDInfoID: getMaxIdForArray(masterData.RecipeDefinitionDyeingInfos, "RecipeDInfoID"),
                ...
                masterData.RecipeDefinitionDyeingInfos.push(newChildItem);
                isRemovable = false;
                initDefDyeingInfoTable();
                $tblDefDyeingInfoEl.bootstrapTable('load', masterData.RecipeDefinitionDyeingInfos);
            }
            else if (dpName == 'Wash') {
                for (var i = 0; i < masterData.RecipeRequestChilds.length; i++) {
                    if (masterData.RecipeRequestChilds[i].RecipeOn === false) {
                        masterData.RecipeRequestChilds[i].RecipeOn = true;
                    }
                }
                
                $tblChildFabricId.bootstrapTable("load", masterData.RecipeRequestChilds);
        ...
        $formEl.find("#btnAddItem").on("click", function (e) {
            ...
            if ((masterData.DPName == 'Single Part' || masterData.DPName == 'Overdye') && masterData.RecipeDefinitionDyeingInfos.length == 1) {
            ...
            var finder = new commonFinder({
                ...
                onMultiselect: function (selectedRecords) {
                    for (var i = 0; i < selectedRecords.length; i++) {
                        var exists = masterData.RecipeRequestChilds.find(function (el) { return el.ConceptID == selectedRecords[i].ConceptID });
                        if (!exists) {
                            masterData.RecipeRequestChilds.push(selectedRecords[i]);
                        }
                    }
        ...
        $formEl.find("#btnAddBatch").on("click", function (e) {
            ...
            var finder = new commonFinder({
               ...
               // data: masterData.DyeingBatchList,
                apiEndPoint: `/api/dyeing-batch/get-dyeing-batch/${colorName}/${conceptNo}`,
                isMultiselect: false,
                allowPaging: true,
                primaryKeyColumn: "DBatchID",
                onSelect: function (selectedRecord) {
                    $formEl.find("#DBatchID").val(selectedRecord.rowData.DBatchID);
                    axios.get(`/api/dyeing-batch/${selectedRecord.rowData.DBatchID}`)
                        .then(function (response) {
                            ...
                            masterData.RecipeRequestChilds = dbList;
                            if (masterData.RecipeRequestChilds != null && masterData.RecipeRequestChilds.length > 0) {
        ...
        $formEl.find("#btnCopyRecipe").on("click", function (e) {
            ...
            var finder = new commonFinder({
                ...
                pageId: pageId,
                apiEndPoint: `/api/rnd-recipe-defination/list-by-groupconcept-compositon-color?fabricComposition=${fComp}&Color=${masterData.ColorID}&GroupConceptNo=${masterData.GroupConceptNo}`,
                isMultiselect: false,
                modalSize: "modal-md",
                top: "2px",
                ...
                onSelect: function (res) {
                    finder.hideModal();
                    recipeCopy = true;
                    recipeReqMasterID = res.rowData.RecipeReqMasterID;
                    recipeID = res.rowData.RecipeID;
                    getDataForRecipe(res.rowData.RecipeReqMasterID, res.rowData.ConceptNo, res.rowData.RecipeNo);
    ...
    function initMasterTable() {
        ...
        var columns = [
            ...
            { field: 'Approved', visible: false },
            { field: 'RecipeStatus', headerText: 'Status', width: 50, visible: status != statusConstants.PENDING },
            { field: 'RecipeReqNo', headerText: 'Recipe Req No', width: 50, visible: status != statusConstants.PENDING },
            { field: 'RecipeReqDate', headerText: 'Recipe Req Date', width: 50, textAlign: 'Center', type: 'date', format: _ch_date_format_1, visible: false },
            { field: 'RecipeReqTime', headerText: 'Recipe Req Date', width: 70, visible: status != statusConstants.PENDING },
            { field: 'DBatchID', headerText: 'Batch ID', width: 50, visible:false},
            { field: 'DBatchNo', headerText: 'Batch No', width: 50 },
            { field: 'GroupConceptNo', headerText: 'Concept No', width: 50 },
            { field: 'ColorName', headerText: 'Color Name', width: 50 },
            { field: 'ColorCode', headerText: 'Color Code', width: 50 },
            //{ field: 'LabDipNo', headerText: 'Lab Dip No', width: 50 },
            { field: 'Buyer', headerText: 'Buyer', width: 50 },
            { field: 'BuyerTeam', headerText: 'Buyer Team', width: 50 },
            { field: 'AcknowledgeDate', headerText: 'Acknowledge Date', width: 50, textAlign: 'Center', type: 'date', format: _ch_date_format_1, visible: status === statusConstants.ACKNOWLEDGE },
            { field: 'DPName', headerText: 'Dyeing Process', width: 40, visible: status != statusConstants.PENDING },
            { field: 'RecipeFor', headerText: 'Recipe For', width: 40, visible: status != statusConstants.PENDING },
            { field: 'IsBDS', headerText: 'Type', width: 20, visible: false },
        ...
        $tblMasterEl = new initEJ2Grid({
            tableId: tblMasterId,
            apiEndPoint: `/api/recipe-request/list?status=${status}`,
            columns: columns,
    ...
    function handleCommands(args) {
        if (args.commandColumn.type == 'Add') {
            ...
            dBatchID = args.rowData.DBatchID;
            getNew(args.rowData.CCColorID, args.rowData.GroupConceptNo, args.rowData.IsBDS, args.rowData.RecipeReqNo, args.rowData.DBatchID);
        }
            ...
            dBatchID = args.rowData.DBatchID;
            getData(args.rowData.RecipeReqMasterID, args.rowData.ConceptNo);
        }
            ...
            columns: [
                {
                    ...
                    events: {
                        'click .remove': function (e, value, row, index) {
                            if (masterData.RecipeRequestChilds.length == 1) {
                            ...
                            showBootboxConfirm("Delete Record.", "Are you sure want to delete this?", function (yes) {
                                if (yes) {
                                    masterData.RecipeRequestChilds.splice(index, 1);
            ...
            columns: [
                {
                    ...
                    events: {
                        'click .remove': function (e, value, row, index) {
                            showBootboxConfirm("Delete Record.", "Are you sure want to delete this?", function (yes) {
                                if (yes) {
                                    masterData.RecipeDefinitionDyeingInfos.splice(index, 1);
                                    $tblDefDyeingInfoEl.bootstrapTable('load', masterData.RecipeDefinitionDyeingInfos);
                                }
                            });
                        }
    ...
    function getColor(rowData) {
        var finder = new commonFinder({
            ...
            onSelect: function (res) {
                ...
                $tblDefDyeingInfoEl.bootstrapTable('updateByUniqueId', { id: rowData.RecipeDInfoID, row: rowData });
            }
        });
        finder.showModal();
    }

    function getNew(ccColorId, grpConceptNo, isBDS, isRework, recipeReqNo, DBatchID) {
        recipeCopy == false;
        masterData = {};
        var url = `/api/recipe-request/new/${ccColorId}/${grpConceptNo}/${isBDS}/${isRework}/${recipeReqNo}/${DBatchID}`;
        axios.get(url)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                ...
                var dpParam = false;
                if (masterData.RecipeRequestChilds.length > 0) dpParam = true;
                ...
                //$tblChildFabricId.bootstrapTable("load", DeepClone(masterData.RecipeRequestChilds));
                $tblChildFabricId.bootstrapTable('load', masterData.RecipeRequestChilds);
                $tblChildFabricId.bootstrapTable('hideLoading');
                ...
                //for collar & cuff
                if (masterData.RecipeRequestChilds[0].SubGroupID != 1) {
                    if (masterData.Composition != null) {
                        var fiberList = [];
                        masterData.FiberPartList.forEach(function (fiber) {
                            if (masterData.Composition.toLowerCase().indexOf(' ' + fiber.text.toLowerCase()) >= 0) {
    ...
    function getData(id, groupConceptNo) {
        var url = `/api/recipe-request/${id}/${groupConceptNo}`;
        axios.get(url)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
    ...
    function loadFabricData() {
        ...
        $tblChildFabricId.bootstrapTable('hideLoading');
        masterData.RecipeDefinitionItemInfos = [Obj];
    }
    ...
    function requestRecipie(isRequest = false, isAck = false) {
        if (dprName == 'Double Part') {
            var IsMatch = true;
            var colorName = masterData.RecipeDefinitionDyeingInfos[0].ColorName;
            for (var i = 1; i < masterData.RecipeDefinitionDyeingInfos.length; i++) {
                if (colorName != masterData.RecipeDefinitionDyeingInfos[i].ColorName) {
                    IsMatch = false;
                    break;
                }
            }
        ...
        data["RecipeDefinitionDyeingInfos"] = masterData.RecipeDefinitionDyeingInfos;
        axios.post("/api/recipe-request/save", data)
            .then(function () {
    ...
    function save(flag) {
        if (recipeCopy == false) {
            var data = masterData.RecipeRequestChilds; // test
            
                data.DBatchID = dBatchID;
            ...
            if (masterData.DPName != "Wash" && masterData.RecipeDefinitionDyeingInfos.length == 0) {
            ...
            data["RecipeRequestChilds"] = masterData.RecipeRequestChilds;
            data["RecipeDefinitionDyeingInfos"] = masterData.RecipeDefinitionDyeingInfos;
            ...
            //if (data.RecipeDefinitionDyeingInfos.length == 0) {
            if (masterData.DPName != "Wash" && data.RecipeDefinitionDyeingInfos.length == 0) {
                toastr.warning('No dyeing information found');
                return;
            }

            var hasError = false;
            for (var i = 0; i < data.RecipeDefinitionDyeingInfos.length; i++) {
                var obj = masterData.FiberPartList.find(y => y.id == data.RecipeDefinitionDyeingInfos[i].FiberPartID);
                if (!obj) {
                    hasError = true;
                    toastr.error("Select fiber part");
                    break;
                } else {
                    data.RecipeDefinitionDyeingInfos[i].FiberPart = obj.text;
                }
            }
            ...
            data.DBatchID = dBatchID;
            axios.post("/api/recipe-request/batchRecipeUpdate", data)
                .then(function () {
                    toastr.success("Saved successfully!");
                    resetTableParams();
                    initMasterTable();
    ...
    function revise() {
        ...
        if (!hasError) {
            ...
            axios.post("/api/recipe-request/revise", data)
                .then(function () {
                    toastr.success("Saved successfully!");
                    resetTableParams();
                    initMasterTable();
    ...
    function acknowledge(unAcknowledgeReason,isAcknowledge) {
        ...
        axios.post("/api/recipe-request/acknowledge", data)
            .then(function () {
                if (isAcknowledge)
    ...
    function createNew() {
        ...
        dBatchID = selectedRecords[0].DBatchID;
        getNew(selectedRecords[0].CCColorID, selectedRecords[0].GroupConceptNo, selectedRecords[0].IsBDS, isRework, recipeReqNo, selectedRecords[0].DBatchID);
    }
}) ();