(function () {
    ...
    $(function () {
        ...
        if (menuParam == 'A') {
            isRequisitionPage = false; isApprovePage = true;
        }
        else if (menuParam == 'R') { isApprovePage = false; isRequisitionPage = true; }
        else {
            isApprovePage = false;
    ...
    function initMasterTable() {
        ...
        if (status == statusConstants.PENDING) { // Data get to the booking table
            columns = [
                ...
                {
                    field: 'YDBatchID', headerText: 'YDBatchID', width: 100, visible: false, isPrimaryKey: true
                },
                {
                    field: 'YDBatchNo', headerText: 'Batch No', width: 100
                },
                {
                ...
                {
                    field: 'ProducedQty', headerText: 'Produced Qty', width: 150
                },
                {
                    field: 'Qty', headerText: 'Dryer Finishing Qty', width: 150
                },
                //{
            ...
            var additionalColumns = [
                {
                    field: 'YDDryerFinishingNo', headerText: 'Dryer Finishing No', width: 150
                },
                {
                    field: 'YDDryerFinishingDate', headerText: 'Dryer Finishing Date', type: 'date', format: _ch_date_format_1, width: 150
                },
                {
        ...
        $tblMasterEl = initEJ2Grid({
            tableId: tblMasterId,
            apiEndPoint: `/api/yd-dryer-finishing/list/${status}/${pageName}`,
            columns: columns,
    ...
    function handleCommands(args) {
        ...
        if (status === statusConstants.PENDING) {
            ...
            getDetails(args.rowData.YDDryerFinishingMasterID);
        }
    }
    ...
    async function initChildTable(data) {
        ...
        if (isRequisitionPage) {
            ...
            var additionalColumns = [
                ...
                { field: 'ProducedQty', headerText: 'Produced Qty', allowEditing: false },
                { field: 'ProducedCone', headerText: 'Produced Cone', allowEditing: false },
                { field: 'Qty', headerText: 'Dryer Finishing Qty', editType: "numericedit", edit: { params: { showSpinButton: false, decimals: 2, min: 1 } } },
                ...
                { field: 'Remarks', headerText: 'Remarks' },
                { field: 'YDProductionChildID', headerText: 'YDProductionChildID', allowEditing: false, visible: false },
            ...
            var additionalColumns = [
                ...
                { field: 'ProducedQty', headerText: 'Produced Qty', allowEditing: false },
                { field: 'ProducedCone', headerText: 'Produced Cone', allowEditing: false },
                { field: 'Qty', headerText: 'Dryer Finishing Qty', allowEditing: true, editType: "numericedit", edit: { params: { showSpinButton: false, decimals: 2, min: 1 } } },
    ...
    function getNew(id) {
        axios.get(`/api/yd-dryer-finishing/new/${id}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                masterData = response.data;
                masterData.YDDryerFinishingNo = masterData.YDDryerFinishingNo;
                masterData.YDDryerFinishingDate = formatDateToDefault(masterData.YDDryerFinishingDate);
    ...
    function getDetails(id) {
        axios.get(`/api/yd-dryer-finishing/${id}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                masterData = response.data;
                masterData.YDDryerFinishinNo = masterData.YDDryerFinishinNo;
                masterData.YDDryerFinishingDate = formatDateToDefault(masterData.YDDryerFinishingDate);
    ...
    function save(IsSendForApprove) {
        ...
        axios.post("/api/yd-dryer-finishing/save", data)
            .then(function () {
    ...
    function isValidChildForm(data) {
        ...
        $.each(data["Childs"], function (i, el) {
            if (el.Qty == "" || el.Qty == null || el.Qty <= 0) {
                toastr.error("Dryer Finishing Qty is required.");
                isValidItemInfo = true;
            }
            else if (el.Qty > el.ReceivedQty) {
                toastr.error("Dryer Finishing Qty must be equal or less than Receive Qty");
                isValidItemInfo = true;
            }
        });
