(function () {
            ...
            columns: [
                {
                    ...
                    formatter: function (value, row, index, field) {
                        if (status === statusConstants.PENDING) {
                            return `<a class="btn btn-xs btn-default add" href="javascript:void(0)" title="New CDA SS Issue">
                                        <i class="fa fa-plus" aria-hidden="true"></i>
                                    </a>`;
                        }
                        else if (status === statusConstants.PARTIALLY_COMPLETED) {
                            return `<a class="btn btn-xs btn-default edit" href="javascript:void(0)" title="Edit CDA SS Issue">
                    ...
                    events: {
                        'click .add': function (e, value, row, index) {
                            e.preventDefault();
                            getNew(row.SSReqMasterID);
                        },
                        'click .edit': function (e, value, row, index) {
                            e.preventDefault();
                            getDetails(row.SSIssueMasterID);
                        }
                    }
                },
                {
    ...
    function getMasterTableData() {
        ...
        var url = `/api/CDA-SS-issue/list?gridType=bootstrap-table&status=${status}&${queryParams}`;
        axios.get(url)
    ...
    function resetForm() {
        ...
        $formEl.find("#SSIssueMasterID").val(-1111);
    ...
    function getNew(SSReqMasterId) {
        axios.get(`/api/CDA-SS-issue/new/${SSReqMasterId}`)
            .then(function (response) {
                $divDetailsEl.fadeIn();
                $divTblEl.fadeOut();
                CDASSIssue = response.data;
                CDASSIssue.SSReqDate = formatDateToDefault(CDASSIssue.SSReqDate);
                CDASSIssue.SSIssueDate = formatDateToDefault(CDASSIssue.SSIssueDate);
                setFormData($formEl, CDASSIssue);
                $tblChildEl.bootstrapTable("load", CDASSIssue.Childs);
    ...
    function save(approve = false, reject = false) {
        ...
        data["Childs"] = CDASSIssue.Childs;
        data.Approve = approve;
        data.Reject = reject;

        axios.post("/api/CDA-SS-issue/save", data)
            .then(function () {
